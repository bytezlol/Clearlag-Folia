...
public class TpChunkCmd extends CommandModule {
    ...
    protected void run(Player player, String[] args) throws WrongCommandArgumentException {
        ...
        final int cx = Integer.parseInt(args[0]);
        final int cz = Integer.parseInt(args[1]);

        final Location regionLoc = new Location(world, (cx << 4) + 8, 0, (cz << 4) + 8);

        ClearLag.scheduler().runAtLocation(regionLoc, task -> world.getChunkAtAsync(cx, cz, true, (Chunk chunk) -> {
            final Block b = chunk.getBlock(8, 0, 8);
            final int x = b.getX();
            final int y = b.getY();
            final int z = b.getZ();

            final Location target = new Location(world, x, world.getHighestBlockYAt(new Location(world, x, y, z)), z);

            ClearLag.scheduler().teleportAsync(player, target);
